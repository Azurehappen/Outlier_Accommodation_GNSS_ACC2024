function [flag,x_post,P_post,b,J_out,num_iter,num_nodes,constraint] =...
    mapRiskAverseSlack(num_constrain,y,H,P,R,J_l,x_prior,td_lambda,num_range)
...
% Add slack variable
ieqLHS = [ieqLHS, -eye(length(ieqRHS))];
lowerbound = [zeros(m+length(ieqRHS),1)]; % lower bound on measurement selection vector b+s
upperbound = [ones(m,1);Inf*ones(length(ieqRHS),1)];  % upper bound on measurement selection vector b+s
...
while num_iter < total_trial
    ...
    % penalty = sum(cost_b)*1000*ones(length(ieqRHS),1);
    penalty = ones(length(ieqRHS),1);
